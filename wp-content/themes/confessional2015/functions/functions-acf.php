...
if (SITE_ENVIRONMENT != 'development'){ // If this is staging or production load ACF declarations
    //require_once(get_stylesheet_directory() . 'plugins/advanced-custom-fields-pro/register-fields.php');
        ...
        add_action( 'admin_menu', 'gi_acf_menu', 9 );
        function gi_acf_menu(){
...
// Set the path to ACF
add_filter('acf/settings/path', 'gi_acf_settings_path');
function gi_acf_settings_path( $path ) {
...
add_filter('acf/settings/dir', 'gi_acf_settings_dir');

function gi_acf_settings_dir( $dir ) {
...
function gi_acf_json_load_point( $path ) {
    unset($path); // kill the default value from ACF Pro
    $path[] = get_stylesheet_directory() . '/functions/plugins/advanced-custom-fields-pro/acf-json';
    return $path;
    }
...
add_filter('get_user_option_edit_acf-field-group_per_page', 'gi_show_all_field_groups', 10, 3);
function gi_show_all_field_groups($result, $option, $user) {
    if((int)$result < 1){
        return 500;
        }
    }
...
add_action( 'admin_menu', 'gi_acf_sync_json', 9.1 );
function gi_acf_sync_json() {
    add_submenu_page(
    ...
    add_submenu_page(
        ...
        array(&$this, 'gi_acf_sync_page'));
    }

/* Create the ACF JSON Sync Page */
add_action('admin_menu', 'register_gi_acf_sync_page');
function register_gi_acf_sync_page(){
    add_submenu_page('options.php', 'Sync Field Groups From JSON', 'Sync Field Groups From JSON', 'manage_options', 'jive-acf-sync-tool', 'display_gi_acf_sync_page');
    }

function display_gi_acf_sync_page(){
    ...
    if(!isset($_POST['jv_sync_field_groups'])){
        echo '
            ...
            // process the json
            echo 'Reading JSON files from: <br>'.gi_acf_json_save_point(false);

            $dir = gi_acf_json_save_point(false);
